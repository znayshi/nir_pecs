...
class GetLastPecsCardUseCaseTest{

    private lateinit var getLastPecsPictogramsUseCase: GetLastPecsCardUseCase
    ...
    fun setUp() {
        ...
        getLastPecsPictogramsUseCase = GetLastPecsCardUseCase(localDataSource)
    }
    ...
    fun `execute get last pictograms with main pictogram id added use case`() = runBlocking {
        ...
        assert(map.containsKey(SaveCardPecsIdUseCase.PICTOGRAM_MAIN))
        assert(map[SaveCardPecsIdUseCase.PICTOGRAM_MAIN]?.pictogramId == MAIN_PICTOGRAM_ID_TO_BE_ADDED)
    }
    ...
    fun `execute get last pictograms with first action pictogram id added use case`() = runBlocking {
        ...
        assert(map.containsKey(SaveCardPecsIdUseCase.PICTOGRAM_FIRST_ACTION))
        assert(map[SaveCardPecsIdUseCase.PICTOGRAM_FIRST_ACTION]?.pictogramId == FIRST_ACTION_ID_TO_BE_ADDED)
    }
    ...
    fun `execute get last pictograms with second action pictogram id added use case`() = runBlocking {
        ...
        assert(map.containsKey(SaveCardPecsIdUseCase.PICTOGRAM_SECOND_ACTION))
        assert(map[SaveCardPecsIdUseCase.PICTOGRAM_SECOND_ACTION]?.pictogramId == SECOND_ACTION_ID_TO_BE_ADDED)
    }
    ...
    fun `execute get last pictograms with attribute pictogram id added use case`() = runBlocking {
        ...
        assert(map.containsKey(SaveCardPecsIdUseCase.PICTOGRAM_ATTRIBUTE))
        assert(map[SaveCardPecsIdUseCase.PICTOGRAM_ATTRIBUTE]?.pictogramId == ATTRIBUTE_ID_TO_BE_ADDED)
    }
    ...
    fun `execute get last pictograms with second attribute pictogram id added use case`() = runBlocking {
        ...
        assert(map.containsKey(SaveCardPecsIdUseCase.PICTOGRAM_SECOND_ATTRIBUTE))
        assert(map[SaveCardPecsIdUseCase.PICTOGRAM_SECOND_ATTRIBUTE]?.pictogramId == SECOND_ATTRIBUTE_ID_TO_BE_ADDED)
    }
}