...
class PictogramFragment : Fragment(), PictogramAdapter.ItemListener {
    ...
    override fun onItemClick(data: PictogramModel) {
        ...
        when {
            GetCardByCategoryUseCase.isAction(data.categoryId) -> {
                if (GetLastPecsCardUseCase.isWantPictogram(data.pictogramId ?: -1)) {
            ...
            GetCardByCategoryUseCase.isAttribute(data.categoryId) -> {
                when {
